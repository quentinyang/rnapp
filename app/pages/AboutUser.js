...
export default class AboutUser extends Component {
            ...
            <TouchableHighlight onPress={() => this.goDetail(rowData)} underlayColor='transparent'>
                <View style={styles.houseItem}>
                    ...
                    <View style={styles.flex}>
                        ...
                        <Text style={{fontSize: 12, marginTop: 2}}>{rowData.get('bedrooms')}室{rowData.get('living_rooms')}厅{rowData.get('bathrooms')}卫 {rowData.get('area')}平 {rowData.get('price')}万</Text>
    ...
    _onEndReached = () => {
        ...
        if(pager.get('total') > pager.get('current_page')*pager.get('per_page')) {
            ActionUtil.setActionWithExtend(actionType.BA_USER_SLIDEUP, {"times": pager.get('current_page')});
